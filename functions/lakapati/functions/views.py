...
class AnalysisView(LoginRequiredMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        farm = 'San Antonio Farm' if str(user_name) == 'og_an' else 'Baliuag'
        result = FarmDetails.objects.filter(user=user_name)

        return render(request, self.template_name, {'result':result, 'farm': farm})